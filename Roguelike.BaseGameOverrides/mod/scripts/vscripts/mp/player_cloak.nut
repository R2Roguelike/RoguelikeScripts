...
void function DisableCloak( entity player, float fadeOut = CLOAK_FADE_OUT )
{
	...
	{
		StatusEffect_Stop( player, player.s.cloakSpeedStatusEffectID )
		delete player.s.cloakSpeedStatusEffectID
	}
...
void function HandleCloakEnd( entity player )
{
	...
	if (Roguelike_HasMod( player, "cloak_speed" ))
	{
		player.s.cloakSpeedStatusEffectID <- StatusEffect_AddEndless( player, eStatusEffect.speed_boost, 0.2 )
	}
