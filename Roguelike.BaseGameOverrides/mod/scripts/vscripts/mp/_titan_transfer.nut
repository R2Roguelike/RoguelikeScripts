...
function CopyWeapons( entity fromEnt, entity toEnt )
{
	...
	// ROGUELIKE - fix for "more than 1 smart ammo weapon"
	// only copy the current primary and its abilities
	array<entity> weapons = fromEnt.GetMainWeapons()
	bool hasLAW = ("lastActiveWeapon" in fromEnt.s)
	if (hasLAW)
	{
		printt("HAS LAW")
		foreach ( i, weapon in weapons )
		{
			if (weapon.GetWeaponClassName() == fromEnt.s.lastActiveWeapon)
			{
				print("transferring " + weapon.GetWeaponClassName())
				entity giveWeapon = fromEnt.TakeWeapon_NoDelete( weapon.GetWeaponClassName() )
				toEnt.GiveExistingWeapon( giveWeapon )
				continue
			}
			print("leaving " + weapon.GetWeaponClassName() + " behind")
			fromEnt.TakeWeapon( weapon.GetWeaponClassName() )
		}
	}
	else if (toEnt.IsPlayer() || IsValid(toEnt.GetBossPlayer()))
	{
		printt("player becoming titan")
		foreach ( i, weapon in weapons )
		{
			print("leaving " + weapon.GetWeaponClassName() + " behind")
			fromEnt.TakeWeapon( weapon.GetWeaponClassName() )
		...
		foreach (string w in playerTitanWeapons)
			toEnt.GiveWeapon( w )
	}
	else
	{
