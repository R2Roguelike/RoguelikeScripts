...
const float LOADOUT_SWITCH_COOLDOWN_PENALTY = 0.0  // 5 second penalty
...
*/
	...
	foreach ( slot,severity in cooldowns )
	{
		...
		{
			severity = CalculateCurrentWeaponCooldownFromStoredTime( player, offhand, file.cooldownData[ offhandName ] )
		}

		highestSeverity = max( severity, highestSeverity )
...
float function CalculateCurrentWeaponCooldownFromStoredTime( entity player, entity offhand, WeaponCooldownData data )
{
	...
	switch( offhand.GetWeaponClassName() )
	{
		...
		case "mp_titanweapon_shoulder_rockets":
		{
			cooldownTime = offhand.GetWeaponSettingFloat( eWeaponVar.charge_cooldown_time )
		}
		break
		...
		case "mp_titanability_power_shot":
		{
			...
			cooldownTime = (maxClipAmmo / refillRate) + drainTime
		}
		break
		...
		case "mp_titanweapon_vortex_shield_ion":// shared energy
		{
			...
			cooldownTime = (maxEnergy / refillRate)
		}
		break
	...
	float startTime = data.timeStored
