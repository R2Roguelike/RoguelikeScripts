...
function VortexSphereDrainHealthForDamage( entity vortexSphere, damage )
{
	...
	entity vortexWeapon = vortexSphere.GetOwnerWeapon()
	if (IsValid( vortexWeapon ) && vortexWeapon.GetWeaponClassName() == "mp_titanability_gun_shield")
	{
		if (Roguelike_HasMod( vortexWeapon.GetWeaponOwner(), "gun_shield_shield"))
		{
			RSE_Apply( vortexWeapon.GetWeaponOwner(), RoguelikeEffect.gun_shield_shield, 1.0, 15.0, 0.0 )
		}
	}
...
bool function CodeCallback_OnVortexHitBullet( entity weapon, entity vortexSphere, var damageInfo )
{
	...
	int teamNum = vortexSphere.GetTeam()
		...
		if ( takesDamage )
		{
			...
			if (Roguelike_HasMod( weapon.GetWeaponOwner(), "anti_shield" ))
				damage *= 5
