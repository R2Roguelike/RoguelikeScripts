...
void function PROTO_FlightCore( entity titan, float flightTime )
{
	...
	if (Roguelike_HasMod( titan, "first_class_flight" ))
	{
		weaponArray.extend(["mp_titanability_hover", "mp_titanweapon_dumbfire_rockets", "mp_titanability_tether_trap" ])
	}

	string otherLoadoutWeapon = ""
	...
	OnThreadEnd(
		function() : ( titan, e, weaponArray, otherLoadoutWeapon )
		{
	...
	if ( titan.IsPlayer() )
	{
		...
		thread FlyerHovers( titan, soundInfo, flightTime, horizontalVelocity, true )
