...
global function ArchonRegisterExecution
...
void function ArchonRegisterExecution()
{
	/*#if SERVER
		RegisterExecutionDataTable( $"datatable/archon_executions.csv" )
		RegisterExecutionPassives(
			{
				GEAR_ARCHON_CHAIN = "execution_archon",
				GEAR_ARCHON_SHIELD = "execution_archon",
				GEAR_ARCHON_FEEDBACK = "execution_archon",
				GEAR_ARCHON_THYLORD = "execution_archon",
				GEAR_ARCHON_SMOKE = "execution_archon"
			}
		)
	#endif*/
}
...
array<entity> function DamageShieldsInRadiusOnEntity( entity weapon, entity inflictor, float radius, float damage )
{
	...
	array < string > shieldClasses = ["mp_titanweapon_vortex_shield", "mp_titanweapon_shock_shield", "mp_titanweapon_heat_shield", "mp_titanability_brute4_bubble_shield", "mp_titanweapon_blast_shield" ] // add shields that are like vortex shield/heat shield to this, they seem to be exceptions?
...
/*void function RegisterExecutionData( asset dataTableAsset )
{
	var dataTable = GetDataTable( dataTableAsset )

	bool hidden			= GetDataTableBool( dataTable, 0, GetDataTableColumnByName( dataTable, "hidden" ) )
	if ( hidden != true )
		return

	string ref			= GetDataTableString( dataTable, 0, GetDataTableColumnByName( dataTable, "ref" ) )
	int itemType		= eItemTypes[ GetDataTableString( dataTable, 0, GetDataTableColumnByName( dataTable, "type" ) ) ]
	string name			= GetDataTableString( dataTable, 0, GetDataTableColumnByName( dataTable, "name" ) )
	string description	= GetDataTableString( dataTable, 0, GetDataTableColumnByName( dataTable, "description" ) )
	asset image			= GetDataTableAsset( dataTable, 0, GetDataTableColumnByName( dataTable, "image" ) )
	int cost			= GetDataTableInt( dataTable, 0, GetDataTableColumnByName( dataTable, "cost" ) )
	bool reqPrime		= GetDataTableBool( dataTable, 0, GetDataTableColumnByName( dataTable, "reqPrime" ) )

	if ( !IsDisabledRef( ref ) )
		return

	CreateTitanExecutionData( 7, itemType, hidden, ref, name, description, description, image, cost, reqPrime )
