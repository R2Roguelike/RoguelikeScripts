...
var function OnWeaponPrimaryAttack_weapon_titanweapon_charge_ball( entity weapon, WeaponPrimaryAttackParams attackParams )
{
	...
	if (Roguelike_HasMod( weaponOwner, "charge_balls" ))
	{
		extraBallAmount++
		angleMultiplier *= 0.7
	}
	...
	for (int i = -extraBallAmount ; i < extraBallAmount+1 ; i++)
	{
		...
		float zapDamage = 1.0 / damageSplitter
	...
	Archon_ConsumeShield( weaponOwner, weapon.GetWeaponSettingInt( eWeaponVar.ammo_per_shot ))
	return 1
}
