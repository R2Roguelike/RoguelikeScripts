...
const FX_STATIC_FEEDBACK_BODY_HUMAN		= $"P_monarchBeam_body_human"
const FX_STATIC_FEEDBACK_BODY_TITAN		= $"P_monarchBeam_body_titan"
...
function FireArcWithTargets( entity weapon, table firstTargetInfo, WeaponPrimaryAttackParams attackParams, muzzleOrigin )
{
	...
	if ( Roguelike_HasMod( player, "chain_reaction" ) )
	{
		if ( player.IsNPC() )
...
array<entity> function GetArcCannonChainTargets( vector fromOrigin, entity fromTarget, table zapInfo )
{
	...
	foreach ( ent in allTargets )
	{
		...
		if ( Roguelike_HasMod( expect entity(zapInfo.player), "chain_reaction" ) )
...
{
	if ( Roguelike_HasMod( expect entity(weapon.GetWeaponOwner()), "chain_reaction" ) )
...
void function ArcCannonOnDamage( entity ent, var damageInfo )
{
	...
	if ( ent.IsPlayer() || ent.IsNPC() )
	{
		...
		if( weapon.HasMod( "static_feedback" ) )
		{
			...
			{
				#if SERVER
					PlayBodyGroupEffect( attacker, weapon, 0.25, FX_STATIC_FEEDBACK_BODY_HUMAN, FX_STATIC_FEEDBACK_BODY_TITAN, damageInfo)
				#endif
...
#if SERVER
function PlayBodyGroupEffect( entity ent, entity weapon, float duration, asset FX_HUMAN, asset FX_TITAN, var damageInfo)
{
	string tag = ""
	asset effect
