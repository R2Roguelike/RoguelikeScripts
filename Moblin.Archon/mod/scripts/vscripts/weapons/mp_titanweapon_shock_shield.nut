...
global function Archon_ConsumeShield
global function Archon_RestoreShield
global function OnWeaponAttemptOffhandSwitch_CheckShield
global function OnWeaponAttemptOffhandSwitch_CheckShield2
...
bool function OnWeaponVortexHitBullet_titanweapon_shock_shield( entity weapon, entity vortexSphere, var damageInfo )
{
		...
		if (Roguelike_HasMod( weapon.GetWeaponOwner(), "shock_shield_shield" ))
		{
			Archon_RestoreShield( weapon.GetWeaponOwner(), int(damage * 0.5) )
		}
...
bool function OnWeaponVortexHitProjectile_titanweapon_shock_shield( entity weapon, entity vortexSphere, entity attacker, entity projectile, vector contactPos )
{
		...
		if (Roguelike_HasMod( weapon.GetWeaponOwner(), "shock_shield_shield" ))
		{
			Archon_RestoreShield( weapon.GetWeaponOwner(), int(projectile.GetProjectileWeaponSettingInt(eWeaponVar.damage_near_value_titanarmor) * 0.5) )
		}
		if ( impactData.refireBehavior == VORTEX_REFIRE_ABSORB )
			return true
...
bool function OnWeaponChargeBegin_titanweapon_shock_shield( entity weapon )
{
	...
	if(Roguelike_HasMod(weaponOwner, "eye_of_the_storm")){
...
bool function OnWeaponAttemptOffhandSwitch_CheckShield2( entity weapon )
{
	return OnWeaponAttemptOffhandSwitch_CheckShield( weapon )
}

bool function OnWeaponAttemptOffhandSwitch_CheckShield( entity weapon )
{
	...
	int activationCost = weapon.GetWeaponSettingInt( eWeaponVar.ammo_min_to_fire )
	
	if (!IsValid(soul))
		return false

	allowSwitch = (soul.GetShieldHealth() >= activationCost)
...
void function Archon_ConsumeShield( entity weaponOwner, int amount )
{
	entity soul = weaponOwner.GetTitanSoul()
	Assert( IsValid( soul ) )

	#if SERVER
	int shieldToConsume = amount
	int remove = minint(soul.GetShieldHealth(), shieldToConsume)
...
void function Archon_RestoreShield( entity weaponOwner, int amount )
{
	entity soul = weaponOwner.GetTitanSoul()
	Assert( IsValid( soul ) )

	#if SERVER
	soul.SetShieldHealth(minint(soul.GetShieldHealthMax(), soul.GetShieldHealth() + amount))
	//soul.nextRegenTime = Time() + GetShieldRegenDelay( soul )
	#endif
...
void function ShockShieldOnDamage( entity ent, var damageInfo )
{
	...
	#if SERVER
	if (DamageInfo_GetDamageSourceIdentifier( damageInfo ) == eDamageSourceId.mp_titanweapon_shock_shield_field)
		ApplyShock( ent, 0.025 )
	else
		ApplyShock( ent, 0.1 )
	#endif
