...
global function ServerCallback_FlyingFightStart
global function ServerCallback_FlyingFightFlight
global function ServerCallback_FlyingFightRefueling
global function ServerCallback_FlyingFightEnd
...
bool flyingFightEnded = false
bool flyingFightFlying = false
float flightStartTime = 0.0
float flightEndTime = 0.0
void function ServerCallback_FlyingFightStart()
{
	thread void function() : ()
	{
		...
		RuiSetFloat2( rui, "msgPos", <0.0, 0.2, 0> )
		RuiSetString( rui, "msgText",  "NO LONG RANGE OPTIONS DETECTED" )
		...
		print("START")
		while (!flyingFightFlying)
			wait 0.001
		while (!flyingFightEnded)
		{
			float percent = GraphCapped( Time(), flightStartTime, flightEndTime, 0, 100 )

			if (flyingFightFlying)
				RuiSetString( rui, "msgText", format("THRUSTER FUEL %.1f%%", 100.0 - percent) )
			else
				RuiSetString( rui, "msgText", format("REFUELING... %.1f%%", percent) )
...
void function ServerCallback_FlyingFightFlight( float endTime )
{
	flyingFightFlying = true
	flightStartTime = Time()
	flightEndTime = endTime
}
void function ServerCallback_FlyingFightRefueling( float endTime )
{
	flyingFightFlying = false
	flightStartTime = Time()
	flightEndTime = endTime
}
void function ServerCallback_FlyingFightEnd()
{
	flyingFightEnded = true
}
