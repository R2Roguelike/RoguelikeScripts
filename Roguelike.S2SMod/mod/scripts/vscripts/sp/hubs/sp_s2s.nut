...
global function __PrintRelativeOrigin
...
void function __PrintRelativeOrigin()
{
	entity maltaRef = file.malta.mover
	printt(GetRelativeDelta( __p().GetOrigin(), maltaRef ))
}

void function MaltaHangar_CustomShipFlightPath( ShipStruct ship, vector position, bool teleport = false )
{
	entity maltaRef = file.malta.mover
	vector pos 		= position
	vector bounds 	= <100, 100, 20>
	vector offset 	= <0, 0, 0>
	if ( teleport )
