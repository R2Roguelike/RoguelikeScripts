...
function IgniteTrap( entity damageArea, var damageInfo, bool isExplosiveBarrel = false )
{
	...
	/*if ( !movingGeo )
		thread FlameOn( inflictor, isWallCanister )
	else
		thread FlameOnMovingGeo( inflictor, isWallCanister )*/
}
...
void function FlameOn( entity inflictor, bool isWallCanister )
{
	inflictor.EndSignal( "OnDestroy")
...
void function FlameOnMovingGeo( entity inflictor, bool isWallCanister )
{
	inflictor.EndSignal( "OnDestroy")
...
void function SpawnFireLine( entity projectile, int projectileCount, entity inflictor, vector origin, vector direction )
{
	...
	projectile.SetAbsAngles( <0,0,0> )
	projectile.proj.savedOrigin = < -999999.0, -999999.0, -999999.0 >

	if (owner.IsPlayer() && Roguelike_HasMod( owner, "offense_canister" ))
...
void function FireTrap_DamagedPlayerOrNPC( entity ent, var damageInfo )
{
	...
	if ( originDistance2D > SLOW_TRAP_RADIUS && !(attacker.IsPlayer() && Roguelike_HasMod( attacker, "offense_canister" )) )
		DamageInfo_SetDamage( damageInfo, 0 )
	else
