	...
	if (weaponOwner.IsPlayer() && Roguelike_HasMod( weaponOwner, "dash_wall" ) && Time() - GetLastDodgeTime() < 0.8)
	{
		...
		for (int i = 0; i < 2; i++)
		{
			...
			if ( projectile )
			{
				...
				EmitSoundOnEntity( projectile, FLAME_WALL_PROJECTILE_SFX )
				weapon.EmitWeaponNpcSound( LOUD_WEAPON_AI_SOUND_RADIUS_MP, 0.5 )
				thread BeginFlameWave( projectile, 0, inflictor, attackParams, dir )
			}
		}
	}
	#endif
...
bool function CreateThermiteWallSegment( entity projectile, int projectileCount, entity inflictor, entity movingGeo, vector pos, vector angles, int waveCount )
{
	...
	if (waveCount != -1 && owner.IsPlayer() && Roguelike_HasMod( owner, "high_wall" ))
	{
		...
		if (waveCount != maxSegments - 1 && waveCount != 0)
		{
			...
			CreateThermiteWallSegment( projectile, projectileCount, inflictor, movingGeo, pos + <0, 0, 200>, angles, -1 )
			CreateThermiteWallSegment( projectile, projectileCount, inflictor, movingGeo, pos + <0, 0, 300>, angles, -1 )
		}
	}
