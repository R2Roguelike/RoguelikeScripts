...
void function OnWeaponChargeEnd_AmpCore( entity weapon )
{
	...
	if (owner.IsPlayer() && Roguelike_HasMod(owner, "never_ending_burst"))
	{
		weapon.SetWeaponBurstFireCount(511) // infinite
		weapon.SetWeaponPrimaryClipCount(50) // half
	}
	else
		weapon.SetWeaponBurstFireCount(weapon.GetWeaponSettingInt(eWeaponVar.burst_fire_count))
