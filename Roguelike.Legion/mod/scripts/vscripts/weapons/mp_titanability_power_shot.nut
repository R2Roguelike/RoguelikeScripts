...
var function OnWeaponPrimaryAttack_power_shot( entity weapon, WeaponPrimaryAttackParams attackParams )
{
	...
	if ( !IsValid( primaryWeapon ) ||
		primaryWeapon.GetWeaponClassName() != "mp_titanweapon_predator_cannon" || primaryWeapon.IsReloading() || weaponOwner.e.ammoSwapPlaying == true )
		return 0
	...
	if ( weaponOwner.IsPlayer() )
	{
		...
		// do not allow swapping loadouts
		foreach (entity w in weaponOwner.GetMainWeapons())
		{
			if (w != primaryWeapon)
				w.AllowUse( false )
		}
...
void function PowerShotCleanup( entity owner, entity weapon, array<string> modNames, array<string> modsToAdd )
{
	if ( IsValid( owner ) && owner.IsPlayer() )
	{
		...
		// do not allow swapping loadouts
		foreach (entity w in owner.GetMainWeapons())
		{
			w.AllowUse( true )
		}
		#endif
